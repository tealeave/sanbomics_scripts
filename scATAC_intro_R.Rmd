...
```{r}
# The mapped peak by cell matrix
counts <- Read10X_h5(filename = "/home/tealeave/projects/scATACseq_data/GSM5723631_Young_HSC_filtered_peak_bc_matrix.h5")
```
...
```{r}
  ...
  sep = c(":", "-"),
  genome = 'mm10',
  fragments = '/home/tealeave/projects/scATACseq_data/GSM5723631_Young_HSC_fragments.tsv.gz',
  min.cells = 10,
  min.features = 200
)
```
...
```{r}
annotations <- GetGRangesFromEnsDb(ensdb = EnsDb.Mmusculus.v79) # get the range of diff genes from mouse db
...
```{r}
# This is because the average length of the DNA length that wrapped around histone is 147. This way we can get fragments that are on nucleosome and the frags that are nucleaosome free.
data <- NucleosomeSignal(object = data) #fragment ratio 147-294: <147 . 
```

```{r}
# adding the TSS enrichment data
data <- TSSEnrichment(object = data, fast = FALSE)
```

```{r}
# These black listed regions have high signal regardless of cell type. We know it's artifacts
...
```{r}
# do a violin plot to determine the threshold
VlnPlot(
  object = data,
...
```{r}
# calculate the quantile and use it to establish cutoffs
...
```{r}
# plot the PCs. PC1 is too highly correlated, it is an artifact.
DepthCor(data)
```

```{r}
# again skip the first 1 because it is an artifact and just use dim 2:30
data <- RunUMAP(object = data, reduction = 'lsi', dims = 2:30)
# clustering
data <- FindNeighbors(object = data, reduction = 'lsi', dims = 2:30)
# they recommend to use algo 3
...
```{r}
young <- import_atac("/home/tealeave/projects/scATACseq_data/GSM5723631_Young_HSC_filtered_peak_bc_matrix.h5",
         '/home/tealeave/projects/scATACseq_data/GSM5723631_Young_HSC_singlecell.csv.gz',
         '/home/tealeave/projects/scATACseq_data/GSM5723631_Young_HSC_fragments.tsv.gz')

old <- import_atac("/home/tealeave/projects/scATACseq_data/GSM5723632_Aged_HSC_filtered_peak_bc_matrix.h5",
         '/home/tealeave/projects/scATACseq_data/GSM5723632_Aged_HSC_singlecell.csv.gz',
         '/home/tealeave/projects/scATACseq_data/GSM5723632_Aged_HSC_fragments.tsv.gz')
```
